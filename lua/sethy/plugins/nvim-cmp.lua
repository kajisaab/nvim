return {
    ...
    dependencies = {
        ...
        -- add LSP source explicitly to ensure strong integration
    ...
    config = function()
        ...
        local lsp_nvim_cmp = require("cmp_nvim_lsp")

        -- Setup enhanced LSP capabilities for completion support
        local capabilities = vim.lsp.protocol.make_client_capabilities()
        capabilities = lsp_nvim_cmp.default_capabilities(capabilities)

        -- Helpers for snippet and indentation (kept as is)
        ...
        local confirm = function(entry)
            ...
            -- For LSP completions, use Insert behavior to avoid text replacement issues
            if entry.source.name == "nvim_lsp" then
                ...
                local range = completion_item.textEdit and completion_item.textEdit.range
                local diff_after = 0
                if range then
                    diff_after = math.max(0, range["end"].character + 1) - entry.context.cursor.col
                end
        ...
        -- Load vscode style snippets from friendly-snippets plugin
        ...
        cmp.setup({
            ...
            sources = cmp.config.sources({
                { name = "luasnip" },
                {
                    ...
                    entry_filter = function(entry, ctx)
                        -- Show all LSP completions including imports
                        return true
                    end,
                },
            ...
            mapping = cmp.mapping.preset.insert({
                ...
                -- <C-d> removed - used for multi-cursor (select next occurrence)
                -- Use <C-e> to close docs or <C-y> to confirm
                ...
                format = function(entry, vim_item)
                    ...
                    if entry.source.name == "nvim_lsp" then
                        ...
                        if completion_item.detail then
                            vim_item.menu = string.format("[%s]", completion_item.detail)
                        else
                            vim_item.menu = "[LSP]"
                        end
                    else
            ...
            capabilities = capabilities,  -- Pass enhanced LSP capabilities
        })

        -- Ghost text toggle based on context (kept as is)
